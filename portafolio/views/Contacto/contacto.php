<div class="p-5" style="background-color:rgba(90, 0, 0, 0.4);">
    <div class="container" style="color:white">
        <h2 class="lead" style="font-size:35px;text-align:center">Medios de contacto / Otros trabajos</h2>
        <hr class="my-2">
        
    </div>

    <div class="row gx-3" style=" margin-left:80px;text-align:center" >

        <div class="card" style="width: 15rem;background-color:rgba(90, 0, 0, 0.4);color:white">
            <img src="
            data:image/jpeg;base64," class="card-img-top" alt="correo" style="height:15rem">
            <div class="card-body">
                <p class="card-text"style="text-transform:uppercase">telefono / Whatsapp de contacto</p>
                <p>3017042671</p>
            </div>
        </div>    

        <div class="card" style="width: 15rem;background-color:rgba(90, 0, 0, 0.4);color:white">
            <img src="https://files.jotform.com/jotformapps/831b690580156421a3b075e8387f9799.jpeg" class="card-img-top" alt="correo" style="text-transform:uppercase;height:15rem">
            <div class="card-body">
                <p class="card-text" style="text-transform:uppercase">Correo e-mail de contacto</p>
                <p>lslinares96@misena.edu.co</p>
            </div>
        </div>
        
        <div class="card" style="width: 15rem;background-color:rgba(90, 0, 0, 0.4);text-transform:uppercase;color:white">
            <img src="https://www.kindpng.com/picc/m/128-1280192_github-logo-png-github-png-transparent-png.png" class="card-img-top" alt="Github" style="height:15rem">
            <div class="card-body">
                <p class="card-text">Link del repositorio principal.</p>
                <a href="https://github.com/lilclums/repphp" class="btn btn-danger" style="margin:5px">Repositorio</a>
                <a href="https://github.com/lilclums" class="btn btn-success" style="margin:5px">Usuario</a>
            </div>
        </div>
    
       <div class="card" style="width: 15rem;background-color:rgba(90, 0, 0, 0.4);text-transform:uppercase;color:white">
            <img src="https://cdn.pixabay.com/photo/2017/08/22/11/56/linked-in-2668700_960_720.png" class="card-img-top" alt="linkedIn" style="height:15rem">
            <div class="card-body">
                <p class="card-text">Cuenta de linkedIn.</p>
                <a href="https://www.linkedin.com/in/leyne-sarena-linares-rodriguez-190594208/" class="btn btn-primary" style="">Usuario</a>
            </div>
        </div>
    </div>

</div>
